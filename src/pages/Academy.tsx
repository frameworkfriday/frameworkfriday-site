      ...
      <section className="py-12 sm:py-20 md:py-28 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <a
            ...
            className="mt-6 sm:mt-8 inline-flex items-center justify-center gap-2 bg-gray-950 text-white font-semibold px-8 py-3.5 sm:py-4 rounded-lg hover:bg-gray-800 transition-all shadow-md w-full sm:w-auto"
          >
            Explore the Academy
            <ArrowRight size={18} />
          </a>
      ...
      <section className="py-12 sm:py-20 md:py-24 px-4 bg-warm-50">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6 sm:gap-8">
          {[
            {
              title: "Format",
              description: "5 live sessions with guided exercises and real-world application",
            },
            {
              title: "Focus",
              description: "Strategy, workflow mapping, ROI, tool selection, and implementation planning",
            },
            {
              title: "Outcome",
              description: "A team aligned on AI strategy with a clear 90-day action plan",
            },
          ].map((item) => (
            <div
              key={item.title}
              className="card-hover border border-gray-200 bg-white rounded-2xl p-6 sm:p-8 text-center"
            >
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3 sm:space-y-4">
            {sessions.map((session) => (
              <div
                key={session.number}
                className="card-hover flex gap-4 sm:gap-5 items-start border border-gray-200 rounded-2xl p-5 sm:p-6 bg-white"
              >
                <span className="flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 bg-primary text-white rounded-xl flex items-center justify-center font-bold text-sm">
                  {session.number}
                </span>
                <div>
